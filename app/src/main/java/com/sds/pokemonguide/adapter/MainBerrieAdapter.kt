...
class MainBerrieAdapter() : PagingDataAdapter<Result, MainBerrieAdapter.ViewHolder>(DataDifferntiator) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val everyBerrieName = getItem(position)?.name

        holder.itemView.findViewById<TextView>(R.id.id_item_name).text = everyBerrieName!!.replace("-", " ")

        Glide.with(holder.itemView.findViewById<ImageView>(R.id.id_item_image).context)
            .load(CONSTANTS.getBerrySpritesByName(everyBerrieName))
    ...
    fun setOnItemClickListenerUrlMainBerrie(listener: (String) -> Unit) {
