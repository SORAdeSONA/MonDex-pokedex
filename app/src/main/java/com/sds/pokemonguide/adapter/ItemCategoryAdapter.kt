...
import com.sds.pokemonguide.model.ItemCategoryItemsList

class ItemCategoryAdapter : RecyclerView.Adapter<ItemCategoryAdapter.DataViewHolder>() {

    private var items: ArrayList<ItemCategoryItemsList> = ArrayList()
    ...
    fun setOnItemClickListenerUrlCategory(listener: (String) -> Unit) {
        onItemClickListenerUrl = listener
    }

    fun addItems(itemsFromItemCategoryFragment: List<ItemCategoryItemsList>) {
        this.items.apply {
            clear()
