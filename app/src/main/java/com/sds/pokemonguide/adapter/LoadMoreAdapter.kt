...
import com.sds.pokemonguide.databinding.LoadMoreBinding
import com.sds.pokemonguide.status.LoadingStatus
import com.sds.pokemonguide.viewmodel.PokemonViewModel

class LoadMoreAdapter(private val viewModel: PokemonViewModel, private val retry: () -> Unit) : LoadStateAdapter<LoadMoreAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(retry: () -> Unit) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun setData(state: LoadState) {
            binding.apply {
                prgBarLoadMore.isInvisible = state is LoadState.NotLoading
