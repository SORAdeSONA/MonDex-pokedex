...
import com.sds.pokemonguide.model.ItemsFlingEffectList

class ItemFlingEffectAdapter : RecyclerView.Adapter<ItemFlingEffectAdapter.DataViewHolder>() {

    private var items: ArrayList<ItemsFlingEffectList> = ArrayList()
    ...
    fun setOnItemClickListenerUrlFlingEffect(listener: (String) -> Unit) {
        onItemClickListenerUrl = listener
    }

    fun addItems(itemsFromItemCategoryFragment: List<ItemsFlingEffectList>) {
        this.items.apply {
            clear()
            addAll(itemsFromItemCategoryFragment)
